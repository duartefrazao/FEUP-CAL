...
void test_performance_dijkstra() {
	...
	for (int n = 10; n <= 50; n += 10) {
		...
		status.open ("status.txt", std::ios_base::app);
		status << "Dijkstra processing grid " << n << " x " << n << " average time (micro-seconds)=" << (elapsed / (n*n)) << endl;
		status.close();

		string file = "grid" + to_string(n) ;
		string filename= file + ".txt";


		 grid.open (filename, std::ios_base::trunc);

		 for(Vertex<pair<int,int>>*  x: g.getVertexSet()){
			grid <<"("<< x->getInfo().first << ","<< x->getInfo().second << ") -> ";
			for(Edge<pair<int,int>> e:x->getAdj()){
				grid << "("<< e.getDest()->getInfo().first << ","<< e.getDest()->getInfo().second<< "), ";
			}
		 }
		 grid.close();
...
void test_bellmanFord() {
	...
	for (int n = 10; n <= 20; n += 10) {
		Graph< pair<int,int> > g;
		cout << "Bellman Ford generating grid " << n << " x " << n << " ..." << endl;
		geneateRandomGridGraph(n, g);
		cout << "Bellman Ford processing grid " << n << " x " << n << " ..." << endl;
		auto start = std::chrono::high_resolution_clock::now();
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				g.bellmanFordShortestPath(make_pair(i,j));
		auto finish = std::chrono::high_resolution_clock::now();
		auto elapsed = chrono::duration_cast<chrono::microseconds>(finish - start).count();


		status.open ("status.txt", std::ios_base::app);
		status << "Bellman Ford processing grid " << n << " x " << n << " average time (micro-seconds)=" << (elapsed / (n*n)) << endl;
		status.close();
...
void performance_edit_distance(){
    srand(time(NULL));
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(performance_edit_distance));
